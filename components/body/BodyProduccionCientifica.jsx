...
export { BodyProduccionCientifica };

function ItemPaper(props) {
    return (<div className="col-md-6 pnl-box-info-link">
        <a href={`/${props.language}/investigacion/${props.tipo}/${props.id}`} aria-label="link ver prodcientf" data-toggle="tooltip" data-placement="bottom" title={props.titulo.trim()}>
            <div className="pnl-box-sctf-prod">
                <div className="row">
                    <h5 className="pnl-box-sect-title">{props.titulo.trim()}</h5>
                </div>
                <div className="row pnl-box-body">
                    <p>{props.autores.trim()}</p>
                </div>
                <div className="row pnl-box-body-subj">
                    <p><span className="badge bg-secondary bg-year-pc">{props.anio.trim()}</span>&nbsp;&nbsp;<span className="badge bg-success bg-type-pc">{props.tipo === "articulo" ? "Artículo" : "Ponencia"}</span></p>
                </div>
            </div>
        </a>
    </div>)
...
function BodyProduccionCientifica(data) {
    ...
    const [listSctPapsFilter, setListSctPapsFilter] = useState([]);
    const [listSubAreas, setListSubAreas] = useState([]);
    const [listSubAreasEspcfs, setListSubAreasEspcfs] = useState([]);
    const [estdListSctPaps, setEstdListSctPaps] = useState(false);
    ...
    useEffect(() => {
        ...
        setCurrentItems(listSctPapsFilter.slice(itemOffset, endOffset));
        setPageCount(Math.ceil(listSctPapsFilter.length / itemsPerPage));
        setEstdListSctPaps(listSctPapsFilter.length > 0);
    }, [itemOffset, itemsPerPage, listSctPapsFilter, endOffset]);

    const handlePageClick = (event) => {
        const newOffset = (event.selected * itemsPerPage) % listSctPapsFilter.length;
    ...
    async function getDataOfSubAreasSpecfById(id_value) {
        var dataLst = [];
        if (id_value !== "") {
            dataLst = await make_external_request_ws(`${WS_LIST_ITEMS_SA_CBX_BY_ID_SGA}${id_value}`);
    ...
    function getAuthorsList(dataAuthors) {
        var authors = '';
    ...
    function getDataPapersAndExposition(data1, data2) {
            ...
            if (data1.length > 0) {
                data1.map((item) => {
                    dataTemp = {
                        ...
                        autores: getAuthorsList(item.autores),
                        anio: item.anio.trim(),
    ...
    const listPaperPanel = (dataItems) => {
        return (
            dataItems.map((item) => {
                return (<ItemPaper key={uuidv4()} tipo={item.tipo} id={item.id} titulo={item.titulo} autores={item.autores} anio={item.anio} language={data.language} />);
            })
        )
    }
    ...
    async function getDataFilterByParameters() {
        ...
        var selectRec = document.getElementById('slct-recurso'), selectArea = document.getElementById('slct-area-conc'),
            selectSubArea = document.getElementById('slct-subarea-conc'), selectSubAreaEspcf = document.getElementById('slct-subarea-esp-conc'), selectYear = document.getElementById('slct-anio'),
            valueRec = selectRec.options[selectRec.selectedIndex].value, parameters = '';
        ...
        switch (valueRec) {
            case "TOD":
                lstBs = await make_external_request_ws(WS_LIST_ITEMS_FILTER_SGA + "tipo=3" + parameters);
                lstChptrs = await make_external_request_ws(WS_LIST_ITEMS_FILTER_SGA + "tipo=4" + parameters);
                resCons = getDataPapersAndExposition(lstBs.data, lstChptrs.data);
                break;
            case "ART":
                lstBs = await make_external_request_ws(WS_LIST_ITEMS_FILTER_SGA + "tipo=3" + parameters);
                resCons = getDataPapersAndExposition(lstBs.data, null);
                break;
            case "PON":
                lstChptrs = await make_external_request_ws(WS_LIST_ITEMS_FILTER_SGA + "tipo=4" + parameters);
                resCons = getDataPapersAndExposition(null, lstChptrs.data);
                break;
        }

        if (resCons.length > 0) {
            setListSctPapsFilter(resCons);
            setItemOffset(0);
            setEndOffset(itemOffset + itemsPerPage);
            setCurrentItems(resCons.slice(itemOffset, endOffset));
            setPageCount(Math.ceil(resCons.length / itemsPerPage));
            setEstdListSctPaps(resCons.length > 0);
        } else {
            setListSctPapsFilter([]);
            setItemOffset(0);
            setEndOffset(0);
            setCurrentItems([]);
            setPageCount(0);
            setEstdListSctPaps(false);
        }
        ...
        <div className="row g-0">
            ...
            <div className="col-md-12 mt-1">
                <div className="alert alert-success message-lang" role="alert">
                    {data.language === "es" ? "El contenido de la producción científica solo esta disponible en español" : (data.language === "en" ? "The content of the scientific production is only available in Spanish" :
                        "O conteúdo da produção científica só está disponível em espanhol")}
                </div>
            </div>
                ...
                <form className="form-card-filter frm-filter" onSubmit={handleSubmit(onSubmitFormBusqueda)}>
                    <div className="row d-flex align-items-center text-left">
                        <div className="form-group col-12 col-sm-12 col-md-12 col-lg-12 col-xl-10 col-xxl-10 flex-column d-flex">
                            ...
                            <input type="text" id="txtbusqueda" className="form-control mt-1" name="ftBusqueda" onChange={handleChange}
                                placeholder={data.language === "es" ? "Búsqueda de artículos y ponencias..." : (data.language === "en" ? "Search for articles and papers" : "Pesquisa de artigos e documentos")} maxLength={200} value={fraseBusq} />
                        </div>
                        <div className="form-group col-6 col-sm-6 col-md-3 col-lg-2 col-xl-2 col-xxl-2 flex-column d-flex">
                            <label className="form-control-label px-3" htmlFor="slct-anio">{data.language === "es" ? "Año" : (data.language === "en" ? "Year" : "Ano")}</label>
                        ...
                        <div className="form-group col-6 col-sm-6 col-md-3 col-lg-2 col-xl-2 col-xxl-2 flex-column d-flex">
                            <label className="form-control-label px-3" htmlFor="slct-recurso">{data.language === "es" ? "Recurso" : (data.language === "en" ? "Resource" : "Recursos")}</label>
                            <select name="ftRecurso" className="form-select form-select-lg" id="slct-recurso">
                                <option value="ART">{data.language === "es" ? "Artículo" : (data.language === "en" ? "Article" : "Artigo")}</option>
                                <option value="PON">{data.language === "es" ? "Ponencia" : (data.language === "en" ? "Presentation" : "Papel")}</option>
                                <option value="TOD">{data.language === "en" ? "All" : "Todos"}</option>
                            </select>
                        </div>
            ...
            <div className="col-md-12 w-100 mt-2">
                <div className="row g-2">
                    {
                        estdListSctPaps ? (<>{currentItems && listPaperPanel(currentItems)}</>) : (
                            isSubmitting ? (<div className="col-md-12 mt-3 pnl-form-filter-news">
                                <h2 className="title-cont-page text-center mt-2">
                                    <span className="spinner-border spinner-border-sm mr-1"></span>&nbsp;&nbsp;{data.language === "es" ? "Buscando información en la base de conocimientos..." :
                                        (data.language === "en" ? "Searching for information in the knowledge base..." : "Procurar informações na base de conhecimentos...")}
                                </h2>
                                ...
                                <h2 className="msg-pnl-search text-center mt-2"><i className="fa fa-book" aria-hidden="true"></i> {data.language === "es" ? "Artículos científicos y ponencias de los docentes e investigadores de la universidad." :
                                    (data.language === "en" ? "Scientific articles and papers by university lecturers and researchers." : "Artigos e trabalhos científicos de professores universitários e investigadores.")}</h2>
                            </div>))
                        )
                    }
                ...
                <div className="row mt-3">
                    <div className="col-md-12 d-flex justify-content-center">
                        {(estdListSctPaps && listSctPapsFilter.length > itemsPerPage) && <ReactPaginate
                            breakLabel="..."
                            nextLabel=">"
