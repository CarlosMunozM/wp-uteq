...
import { RGL_NRM_DOCS_FOLDER, WS_LIST_SHORT_FILES_UNIV_BY_TYPE } from 'config';
import React, { useState, useEffect, useMemo } from "react";
import axios from 'axios';

export { BodyReglmNorm };
...
function BodyReglmNorm(data) {
    ...
    useEffect(() => {
        (async () => {
            const result = await make_request_ws(`${WS_LIST_SHORT_FILES_UNIV_BY_TYPE}REGNM`);
            setDatatbl((result.data !== null && result.data !== "") ? result.data.sort((a, b) => (a.arAnio > b.arAnio) ? -1 : 1) : []);
        })();
    }, []);
    ...
    const columns = (language) => [
        ...
        {
            name: (language === "es" ? "Tipo (Gestión, Académica)" : (language === "en" ? "Type (Management, Academic)" : "Tipo (Gestão, Académico)")),
            selector: row => (language === "es" ? row.arFase.trim() : (language === "en" ? row.arFaseEn.trim() : row.arFasePt.trim())),
            sortable: true,
            width: '25%',
            filterable: true
        },
    ...
    const DownloadFiles = row => (<>
        <a href={`${RGL_NRM_DOCS_FOLDER}${row.arUrlDocumento}`} target="_blank" className="btn-table" data-toggle="tooltip" data-placement="bottom" title={data.language === "es" ? "Descargar documento" : (data.language === "en" ? "Download document" : "Baixar documento")}>
            <i className="fa fa-arrow-circle-o-down fa-2x" aria-hidden="true"></i>
        </a>
    </>);

    const PreviewFiles = row => (<>
        <a href="#" onClick={() => { row.arUrlDocumento.includes(".pdf") && window.open(`${RGL_NRM_DOCS_FOLDER}${row.arUrlDocumento}`, "_blank", "fullscreen=yes"); return false }} target="_blank" className="btn-table" data-toggle="tooltip" data-placement="bottom" title={data.language === "es" ? "Visualizar documento" : (data.language === "en" ? "View document" : "Exibir documento")}>
    ...
    return (<>
        <div className="row">
            <h2 className="title-cont-page text-center mt-2">{data.language === "es" ? "Listado de reglamentos y normativas" : (data.language === "en" ? "List of regulations and rules" : "Lista de regulamentos e normas")}</h2>
