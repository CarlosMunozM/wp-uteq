...
function PanelNews(data_news, list_ctgs, cod_dep) {
    ...
    const handleMultiChangeSelCatgs = (selectedOptions) => {
        // selectedOptions ya es el nuevo array con los elementos actualizados (o null si se vació)
        setListCatgSelc(selectedOptions || []);
    };

    const handleMultiChangeSelYears = (selectedOptions) => {
        setListAnioSelc(selectedOptions || []);
    };
    ...
    const onSubmitFormFilterNews = async () => {
        ...
        // 1. SI HAY FILTROS SELECCIONADOS: Ejecuta reCAPTCHA y consulta la API
        if ((listCatgSelc && listCatgSelc.length > 0) || (listAnioSelc && listAnioSelc.length > 0)) {

            // Limpiamos la lista actual solo si vamos a filtrar en el servidor
            setListNewsFilter([]);
                        ...
                        .then(async (token) => {
                            try {
                                await axios.post(`${WS_VALIDATE_FORM}${token}`).then(async function (response) {
                                    ...
                                    setMesgForm(error.message || error);
                                    setTpmsgForm(1);
                                    setIsSubmitting(false);
                                })
                            } catch (error) {
            ...
            // 2. SI AMBOS FILTROS ESTÁN VACÍOS: Restaura las noticias iniciales (data_news)
            const initialNews = (data_news !== null && data_news !== undefined && data_news !== "") ? data_news : [];

            setListNewsFilter(initialNews);
            setItemOffset(0); // Regresa la paginación a la página 1
            setEstdListNews(initialNews.length > 0);
            setIsSubmitting(false);
        }
    ...
    const onSubmitFormFilterNews2 = async () => {
